...
The simplified canonical path should adhere to the following rules:
    ...
    It should exclude any single or double periods used to denote current or parent 
    directories.
...
def simplifyPath(path):
    ...
    for i in range(len(split_string)):
        ...
        if split_string[i] == ".." and len(stack) == 0:
            continue
        if split_string[i] == "":
            #ignore
            continue
        elif split_string[i] == "...":
            stack.append(split_string[i])
        elif split_string[i] == "..":
            #go back in stack
            stack.pop()
        elif split_string[i] == ".":
